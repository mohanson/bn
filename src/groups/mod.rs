...
impl Copy for EllCoeffs {}

impl Clone for EllCoeffs {
    fn clone(&self) -> Self {
        EllCoeffs {
            ell_0: self.ell_0,
            ell_vw: self.ell_vw,
...
impl AffineG<G2Params> {
    ...
    pub fn precompute(&self) -> G2Precomp {
        ...
        let mut coeffs_index = 0;

        let q_neg = self.neg();
        for i in ATE_LOOP_COUNT_NAF.iter() {
            coeffs[coeffs_index] = r.doubling_step_for_flipped_miller_loop();
            coeffs_index += 1;

            if *i == 1 {
                coeffs[coeffs_index] = r.mixed_addition_step_for_flipped_miller_loop(self);
                coeffs_index += 1;
            }
            if *i == 3 {
                coeffs[coeffs_index] = r.mixed_addition_step_for_flipped_miller_loop(&q_neg);
                coeffs_index += 1;
            }
        }
        ...
        coeffs[coeffs_index] = r.mixed_addition_step_for_flipped_miller_loop(&q1);
        coeffs_index += 1;
        coeffs[coeffs_index] = r.mixed_addition_step_for_flipped_miller_loop(&q2);
