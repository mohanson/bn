...
fn gen_u256(s: &str) -> U256 {
    U256::from_slice(&hex_decode(s.strip_prefix("0x").unwrap())).unwrap()
}
...
fn fqstr(a: &Fq) -> String {
    return format!("Fq(0x{:032x}{:032x})", a.0 .0[1], a.0 .0[0]);
}

fn fq2str(a: &Fq2) -> String {
    return format!("Fq2({}, {})", fqstr(a.real()), fqstr(a.imaginary()));
}

fn fq6str(a: &Fq6) -> String {
    return format!(
        "Fq6({}, {}, {})",
        fq2str(&a.c0),
        fq2str(&a.c1),
...
fn fq12str(a: &Fq12) -> String {
    return format!("Fq12({}, {})", fq6str(&a.c0), fq6str(&a.c1));
}
